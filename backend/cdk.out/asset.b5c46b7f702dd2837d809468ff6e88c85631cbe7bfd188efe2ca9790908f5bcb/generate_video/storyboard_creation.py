...
def _call_openai_storyboard(api_key: str, context: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # Build a readable context summary instead of embedding raw JSON
    scene_lines = []
    for s in scenes[:10]:
        try:
            sid = s.get("id")
            ...
            scene_lines.append(f"- scene {sid}: '{desc}' (duration_s={dur})")
        except Exception:
            pass
    images_brief = _brief_images(images)
    images_line = ", ".join([f"scene {i.get('scene_id')}: {i.get('s3')}" for i in images_brief]) or "none"
    text_samples = []
    for t in texts[:3]:
    ...
    user = (
        ...
        f"Summary: {summary}\n"
        f"Storytelling: {storytelling}\n"
        f"Scenes (up to 10 shown):\n" + ("\n".join(scene_lines) or "- none") + "\n"
        f"Assets - images: {images_line}\n"
        f"Assets - text samples (up to 3):\n" + ("\n".join(text_samples) or "- none") + "\n"
        "Requirements: defaults width=1920, height=1080, fps=30, crf=18, transitions=none, "
        f"music={DEFAULT_MUSIC_S3}, music_volume=0.12."
    )
